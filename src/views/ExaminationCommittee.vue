  ...
  <div class="examination-committee">
    ...
    <div class="examination-cards">
      ...
      <div v-else class="cards-grid">
        <div v-if="diagnoses.length > 0" class="cards-grid">
          <div v-for="diagnosis in diagnoses" :key="diagnosis.id" class="card">
            ...
            <div class="card-content">
              ...
              <div class="info-row" v-if="diagnosis.medicalStaff?.user">
                <span class="label">الطبيب:</span>
                <span class="value">
                  {{ diagnosis.medicalStaff.user.firstName || "" }}
                  {{ diagnosis.medicalStaff.user.secondName || "" }}
                  {{ diagnosis.medicalStaff.user.thirdName || "" }}
                </span>
              </div>
              <div class="info-row" v-if="diagnosis.staffSections">
                <span class="label">القسم:</span>
                <span class="value">{{ diagnosis.staffSections.sectionName || "غير متوفر" }}</span>
              </div>
            ...
            <div class="card-actions">
              ...
              <button @click="openReplyModal(diagnosis)" class="reply-btn">
        ...
        <form @submit.prevent="handleSubmit" class="examination-form">
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="reviewerId" v-model="formData.reviewerId" required>
            ...
            <div class="form-group">
              ...
              <select id="medicalStaffId" v-model="formData.medicalStaffId" required @change="onDoctorChange">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="text" id="bloodPressure" v-model="formData.bloodPressure" />
            </div>
            <div class="form-group">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="text" id="oxygenSaturation" v-model="formData.oxygenSaturation" />
            </div>
            <div class="form-group">
              <label for="temperature">درجة الحرارة</label>
              <input type="text" id="temperature" v-model="formData.temperature" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label for="pulse">النبض</label>
              <input type="text" id="pulse" v-model="formData.pulse" />
            </div>
            <div class="form-group">
              <label for="staffSectionId">القسم</label>
              <input type="text" id="staffSectionId" v-model="formData.staffSectionId" readonly />
            </div>
          </div>
          <div class="form-group">
            <label for="chronicDiseases">الأمراض المزمنة</label>
            <textarea id="chronicDiseases" v-model="formData.chronicDiseases" rows="3"></textarea>
          </div>
          <div class="form-group">
            <label for="communicableDiseases">الأمراض المعدية</label>
            <textarea id="communicableDiseases" v-model="formData.communicableDiseases" rows="3"></textarea>
          </div>
          <div class="form-group">
            <label for="allergies">الحساسية</label>
            <textarea id="allergies" v-model="formData.allergies" rows="3"></textarea>
          </div>
          <div class="form-group">
            <label for="drugAllergies">حساسية الأدوية</label>
            <textarea id="drugAllergies" v-model="formData.drugAllergies" rows="3"></textarea>
          </div>
          <div class="form-group">
            <label for="skinDiseases">أمراض الجلد</label>
            <textarea id="skinDiseases" v-model="formData.skinDiseases" rows="3"></textarea>
          </div>
          <div class="form-group">
            <label for="referredToParty">محول إلى</label>
            <input type="text" id="referredToParty" v-model="formData.referredToParty" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="handleReplySubmit" class="examination-form">
          <div class="form-group">
            ...
            <textarea id="reply" v-model="replyFormData.reply" required rows="4"></textarea>
          </div>
          <div class="form-group">
            <label for="replyImage">صورة الرد</label>
            <input type="file" id="replyImage" @change="handleReplyImageChange" accept="image/*" />
          </div>
          <div v-if="errorMessage" class="error-message">{{ errorMessage }}</div>
          <div v-if="successMessage" class="success-message">{{ successMessage }}</div>
          <div class="form-actions">
            <button type="submit" class="submit-button" :disabled="loading">
              <i class="pi" :class="loading ? 'pi-spin pi-spinner' : 'pi-save'"></i> حفظ
            </button>
            <button type="button" class="cancel-button" @click="closeReplyModal">
              <i class="pi pi-times"></i> إلغاء
            </button>
          </div>
        </form>
      </div>
    ...
    <div v-if="showDiagnosisModal" class="modal-overlay" @click="closeDiagnosisModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h2>تشخيص الطبيب</h2>
          <button class="close-button" @click="closeDiagnosisModal">×</button>
        </div>
        <form @submit.prevent="handleDiagnosisSubmit" class="examination-form">
          <div class="form-group">
            ...
            <div v-for="(input, index) in diagnosisForm.medicalInputs" :key="index" class="input-group">
              <input type="text" v-model="input.value" :placeholder="'المدخل الطبي ' + (index + 1)" />
          ...
          <div class="form-group">
            ...
            <div v-for="(input, index) in diagnosisForm.sectionInputs" :key="index" class="input-group">
              <input type="text" v-model="input.value" :placeholder="'مدخل القسم ' + (index + 1)" />
          ...
          <div class="form-group">
            ...
            <input type="file" id="diagnosisFile" @change="handleDiagnosisFileChange" accept="image/*,.pdf,.doc,.docx" />
          </div>
          <div class="form-group">
            <label for="fileDescription">وصف الملف</label>
            <textarea id="fileDescription" v-model="diagnosisForm.fileDescription" rows="3"></textarea>
          </div>
          <div v-if="errorMessage" class="error-message">{{ errorMessage }}</div>
          <div v-if="successMessage" class="success-message">{{ successMessage }}</div>
          <div class="form-actions">
    ...
    <div v-if="showTransferModal" class="modal-overlay" @click="closeTransferModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h2>تحويل التشخيص</h2>
          <button class="close-button" @click="closeTransferModal">×</button>
        </div>
        <form @submit.prevent="handleTransferSubmit" class="examination-form">
          <div class="form-group">
            <label for="toMedicalStaffId">الطبيب المحول إليه *</label>
            <select id="toMedicalStaffId" v-model="transferForm.toMedicalStaffId" required @change="onTransferDoctorChange">
              <option value="">اختر الطبيب</option>
              <option v-for="staff in medicalStaff" :key="staff.id" :value="staff.id">
                {{ staff.user?.firstName || "" }} {{ staff.user?.secondName || "" }} {{ staff.user?.thirdName || "" }}
              </option>
          ...
          <div class="form-group">
            ...
            <input type="text" id="toStaffSectionsId" v-model="transferForm.toStaffSectionsId" readonly />
          </div>
          <div class="form-group">
            <label for="transferDescription">وصف التحويل</label>
            <textarea id="transferDescription" v-model="transferForm.description" rows="4" placeholder="أدخل سبب التحويل..."></textarea>
          </div>
          <div v-if="errorMessage" class="error-message">{{ errorMessage }}</div>
          <div v-if="successMessage" class="success-message">{{ successMessage }}</div>
          <div class="form-actions">
            <button type="submit" class="submit-button" :disabled="loading">
              <i class="pi" :class="loading ? 'pi-spin pi-spinner' : 'pi-save'"></i> تحويل
            </button>
        ...
        <div class="details-content" v-if="selectedDiagnosis">
          <div class="details-section">
            ...
            <div class="info-row">
              ...
              <span class="value">{{ selectedDiagnosis.reviewer?.bookNumber || "غير متوفر" }}</span>
            </div>
            <div class="info-row">
          ...
          <div class="details-section">
            ...
            <div class="info-row">
              <span class="label">الطبيب:</span>
              <span class="value">
                {{ selectedDiagnosis.medicalStaff?.user?.firstName || "" }}
                {{ selectedDiagnosis.medicalStaff?.user?.secondName || "" }}
                {{ selectedDiagnosis.medicalStaff?.user?.thirdName || "" }}
              </span>
            </div>
            <div class="info-row">
              <span class="label">التخصص:</span>
              <span class="value">{{ selectedDiagnosis.medicalStaff?.specialization || "غير متوفر" }}</span>
            </div>
            <div class="info-row">
              <span class="label">القسم:</span>
              <span class="value">{{ selectedDiagnosis.staffSections?.sectionName || "غير متوفر" }}</span>
            </div>
          </div>
          <div class="details-section">
...
export default {
  ...
  methods: {
    ...
    openReplyModal(diagnosis) {
      if (!diagnosis || !diagnosis.id) {
        this.errorMessage = "خطأ: بيانات الفحص غير صالحة";
        return;
      }
      this.selectedDiagnosis = diagnosis;
      this.replyFormData = {
    ...
    getReviewerName(reviewerId) {
      ...
      return reviewer
        ? `${reviewer.firstName || ""} ${reviewer.secondName || ""} ${reviewer.thirdName || ""} ${reviewer.surName || ""}`.trim()
        : "غير معروف";
    },
  ...
  mounted() {
    Promise.all([this.fetchReviewers(), this.fetchMedicalStaff(), this.fetchDiagnoses()]).catch((error) => {
      this.errorMessage = "حدث خطأ أثناء تحميل البيانات الأولية";
    });
  },
};
</script>
